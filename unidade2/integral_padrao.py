...
def formula_trapezio(x_inicio, x_fim = None, delta_x = None):
...
def formula_simpson_grau_2(x_inicio, x_fim = None, delta_x = None):
    return None

# Funcao para calcular a integral correta
def integral_final(x_inicio, x_fim, erro):
    erro_atual = float("inf")
    integral_ultima = formula_trapezio(x_inicio, x_fim = x_fim)
    n = 1
    iteracoes = 0
    while (erro_atual > erro):
        ...
        for i in range(n):
            a = x_inicio + i * h
            b = a + h
            integral_atual += formula_trapezio(a, x_fim = b)
...
print(integral_final(2, 4, 0.00001))